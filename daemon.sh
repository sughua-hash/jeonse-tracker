...
# 백그라운드 데몬: ① 매일 RUN_AT(기본 10:00, 폰 시간) 이후 그날 수집이 성공할 때까지 RETRY_SEC(기본 10분) 간격 재시도
...
export DAEMON_VER=3   # run_termux.sh 가 logs/daemon.version 과 비교해 구버전 데몬이면 자동 재시작
stop_daemon(){
...
nohup bash -c '
  ...
  since=$(date +%s); last_run=0; last_try=0
  ...
  while true; do
    ...
    # 정기 수집: 예정 시각이 지났고, 오늘 아직 성공하지 못했고, 마지막 시도로부터 RETRY_SEC 이 지났으면 (재시도 포함)
    if [ -n "$run_ts" ] && [ "$now" -ge "$run_ts" ] && [ "$ok" != "$today" ] && [ $((now - last_try)) -ge "$RETRY_SEC" ]; then
      reason="정기"; last_try=$now
    fi
    ...
    if [ -n "$reason" ]; then
      if [ "$reason" != "정기" ] && [ $((now - last_run)) -lt 120 ]; then
        ...
        if [ "$(cat logs/last_success.date 2>/dev/null)" = "$today" ]; then
